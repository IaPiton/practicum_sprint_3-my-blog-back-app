...
class CommentServiceImplTest {
    ...
    class GetCommentByIdTests {
        ...
        void shouldReturnCommentById() {
            when(commentRepository.findById(1L)).thenReturn(testCommentEntity);
            when(commentMapper.toResponse(testCommentEntity)).thenReturn(testCommentResponse);
        ...
        @Test
        @DisplayName("Должен вернуть null, если комментарий не найден")
        void shouldReturnNullWhenCommentNotFound() {
            when(commentRepository.findById(999L)).thenReturn(null);

            CommentResponse result = commentService.getCommentById(999L);

            assertThat(result).isNull();
            verify(commentRepository).findById(999L);
            verify(commentMapper).toResponse(null);
        }
    }
    ...
    class CreateCommentTests {
        ...
        void shouldCreateCommentSuccessfully() {
            ...
            newCommentEntity.setPostId(testCreateRequest.getPostId());
            newCommentEntity.setText(testCreateRequest.getText());
    ...
    class UpdateCommentTests {
        ...
        @Test
        @DisplayName("При обновлении несуществующего комментария должен выбросить исключение")
        void shouldThrowExceptionWhenUpdatingNonExistentComment() {
            when(commentRepository.findById(999L)).thenReturn(null);

            try {
                commentService.updateComment(999L, testUpdateRequest);
        ...
        void shouldUpdateOnlyTextField() {
            ...
            when(commentRepository.findById(1L)).thenReturn(originalComment);
            when(commentMapper.toResponse(any(CommentsEntity.class))).thenReturn(
                    CommentResponse.builder()
                            .id(1L)
                            .postId(100L)
    ...
    class PostExistsTests {
        ...
        @Test
        @DisplayName("Должен корректно обработать null в качестве Id поста")
        void shouldHandleNullPostId() {
            when(commentRepository.postExists(null)).thenReturn(false);

            boolean result = commentService.postExists(null);

            assertThat(result).isFalse();
            verify(commentRepository).postExists(null);
        }
    }
}